...
export const useRemoveRoleFromeMember = () => {
  ...
  const remove = useMutation(api.roles.removRoleFromMember);

  const removeRoleFromMember = useCallback(
    async (
      ...
      try {
        ...
        setStatus("pending");
        const response = await remove({ serverMemberId, roleId });
